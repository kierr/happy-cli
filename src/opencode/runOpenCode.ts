...
import { OpenCodeMcpClient } from './openCodeMcpClient';
...
import type { OpenCodeSessionConfig } from './types';
...
/**
 * Notify connected clients when OpenCode finishes processing and the queue is idle.
...
export async function runOpenCode(opts: {
    ...
    // Log startup options
    logger.debug(`[opencode] Starting with options: startedBy=${opts.startedBy || 'terminal'}`);
    ...
    session.onUserMessage((message) => {
        ...
        if (message.meta?.hasOwnProperty('model')) {
            ...
            logger.debug(`[OpenCode] Model updated from user message: ${messageModel || 'reset to default'}`);
        } else {
            logger.debug(`[OpenCode] User message received with no model override, using current: ${currentModel || 'default'}`);
        }
    ...
    function logActiveHandles(tag: string) {
        ...
        logger.debug(`[opencode][handles] ${tag}: handles=${handles.length} requests=${requests.length}`);
        try {
            const kinds = handles.map((h: any) => (h && h.constructor ? h.constructor.name : typeof h));
            logger.debug(`[opencode][handles] kinds=${JSON.stringify(kinds)}`);
        } catch { }
    }
    ...
    async function handleAbort() {
        ...
        try {
            ...
            if (client.hasActiveSession()) {
                ...
                logger.debug('[OpenCode] Stored session for resume:', storedSessionIdForResume);
            }
    ...
    // Helper: find OpenCode session transcript for a given sessionId
    function findOpenCodeResumeFile(sessionId: string | null): string | null {
        if (!sessionId) return null;
        try {
            const opencodeHomeDir = process.env.OPENCODE_HOME || join(os.homedir(), '.opencode');
            const rootDir = join(opencodeHomeDir, 'sessions');
    ...
    const reasoningProcessor = new ReasoningProcessor((message) => {
        ...
        session.sendOpenCodeMessage(message);
    });
    ...
    client.setHandler((msg) => {
        logger.debug(`[OpenCode] MCP message: ${JSON.stringify(msg)}`);
    ...
    // Start Happy MCP server (HTTP) and prepare STDIO bridge config for OpenCode
    ...
    try {
        logger.debug('[opencode]: client.connect begin');
        await client.connect();
        logger.debug('[opencode]: client.connect done');
        ...
        while (!shouldExit) {
            ...
            if (!message) {
                ...
                if (!batch) {
                    ...
                    if (waitSignal.aborted && !shouldExit) {
                        logger.debug('[opencode]: Wait aborted while idle; ignoring and continuing');
                        continue;
                    }
                    logger.debug(`[opencode]: batch=${!!batch}, shouldExit=${shouldExit}`);
                    break;
                }
                message = batch;
            }
            ...
            if (wasCreated && currentModeHash && message.hash !== currentModeHash) {
                ...
                try {
                    ...
                    nextExperimentalResume = findOpenCodeResumeFile(prevSessionId);
                    if (nextExperimentalResume) {
                        logger.debug(`[OpenCode] Found resume file for session ${prevSessionId}: ${nextExperimentalResume}`);
                        messageBuffer.addMessage('Resuming previous context…', 'status');
                    } else {
                        logger.debug('[OpenCode] No resume file found for previous session');
                    }
                } catch (e) {
                    logger.debug('[OpenCode] Error while searching resume file', e);
                }
                client.clearSession();
                wasCreated = false;
            ...
            try {
                ...
                if (!wasCreated) {
                    const startConfig: OpenCodeSessionConfig = {
                    ...
                    if (nextExperimentalResume) {
                        ...
                        logger.debug('[OpenCode] Using resume file from mode change:', resumeFile);
                    }
                    // Priority 2: Resume from stored abort session
                    else if (storedSessionIdForResume) {
                        const abortResumeFile = findOpenCodeResumeFile(storedSessionIdForResume);
                        if (abortResumeFile) {
                            resumeFile = abortResumeFile;
                            logger.debug('[OpenCode] Using resume file from aborted session:', resumeFile);
                    ...
                    logger.debug('[OpenCode] continueSession response:', response);
                }
            } catch (error) {
                logger.warn('Error in opencode session:', error);
                ...
                if (isAbortError) {
                    ...
                    wasCreated = false;
                    currentModeHash = null;
                    logger.debug('[OpenCode] Marked session as not created after abort for proper resume');
                } else {
                    ...
                    if (client.hasActiveSession()) {
                        storedSessionIdForResume = client.storeSessionForResume();
                        logger.debug('[OpenCode] Stored session after unexpected error:', storedSessionIdForResume);
                    }
                }
            } finally {
        ...
        try {
            logger.debug('[opencode]: sendSessionDeath');
            session.sendSessionDeath();
            logger.debug('[opencode]: flush begin');
            await session.flush();
            logger.debug('[opencode]: flush done');
            logger.debug('[opencode]: session.close begin');
            await session.close();
            logger.debug('[opencode]: session.close done');
        } catch (e) {
            logger.debug('[opencode]: Error while closing session', e);
        }
        logger.debug('[opencode]: client.disconnect begin');
        await client.disconnect();
        logger.debug('[opencode]: client.disconnect done');
        // Stop Happy MCP server
        logger.debug('[opencode]: happyServer.stop');
        happyServer.stop();

        // Clean up ink UI
        if (process.stdin.isTTY) {
            logger.debug('[opencode]: setRawMode(false)');
            try { process.stdin.setRawMode(false); } catch { }
        }
        // Stop reading from stdin so that process can exit
        if (hasTTY) {
            logger.debug('[opencode]: stdin.pause()');
            try { process.stdin.pause(); } catch { }
        }
        // Clear periodic keep-alive to avoid keeping event loop alive
        logger.debug('[opencode]: clearInterval(keepAlive)');
        clearInterval(keepAliveInterval);
        if (inkInstance) {
            logger.debug('[opencode]: inkInstance.unmount()');
            inkInstance.unmount();
        }
        messageBuffer.clear();
